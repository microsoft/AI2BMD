...
class ViSNetModel:
    ...
    def dl_potential_loader(self, frag_data: FragmentData):
        ...
        with torch.cuda.stream(self.stream):
            ...
            energy = energy.detach().cpu().reshape(-1, 1).numpy()
            forces = forces.detach().cpu().reshape(-1, 3).numpy()
...
class ViSNetCalculator(Calculator):
    ...
    def __init__(self, ckpt_path: str, ckpt_type: str,
        ...
        assert self.ckpt_type in [
            "new",
            "old",
        ], "ckpt_type should be new or old"
...
def get_visnet_model(model_path: str, device: str):
    # return ViSNetAsyncCalculator(model_path, device)
